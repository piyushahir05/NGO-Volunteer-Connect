...
exports.getRecommendedVolunteers = async (req, res, next) => {
  try {
   ...
   const ML_URL = process.env.ML_API_URL || 'http://localhost:8000';
    ...
    if (!mlResponse.ok) {
      console.error('[ML Service Error]', mlResponse.status, mlResponse.statusText);
      return res.status(502).json({ message: 'ML service unavailable. Please try again later.' });
    }
