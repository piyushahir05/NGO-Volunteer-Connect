...
import {
  ...
  ArrowLeft, Mail, Clock, Building2
} from 'lucide-react';
      ...
      <div className="relative z-10 max-w-2xl mx-auto space-y-6">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center h-[50vh]">
            ...
            <span className="mt-4 text-xs font-bold tracking-widest text-emerald-600 uppercase">Loading Messages...</span>
          </div>
        ) : (
            ...
            <motion.div variants={fadeUp} className={`${glassCardClass} p-6 md:p-8 relative`}>
              ...
              <div className="relative z-10">
                ...
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/60 border border-white/80 shadow-sm text-emerald-700 text-[10px] font-bold tracking-widest uppercase mb-3 backdrop-blur-md">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> Inbox
                </div>
                <h1 className="text-2xl md:text-3xl font-extrabold text-slate-800 tracking-tight mb-1">Your Messages</h1>
                <p className="text-slate-500 text-sm font-medium">Conversations with organizations you've applied to.</p>
              </div>
            </motion.div>
            ...
            <div className="flex flex-col gap-3">
              {filtered.length === 0 ? (
                <motion.div variants={fadeUp} className={`${glassCardClass} p-14 text-center`}>
                  ...
                  <div className="text-base font-bold text-slate-800 mb-1">No messages yet</div>
                  <p className="text-sm text-slate-400">Organizations you've applied to can message you here.</p>
                </motion.div>
              ) : (
                <AnimatePresence>
                  {filtered.map((n) => {
                    const initials = (n.name || 'N').split(' ').map((w) => w[0]).join('').slice(0, 2).toUpperCase();
                    return (
                      <motion.button
                        key={n._id}
                        variants={fadeUp}
                        onClick={() => navigate(`/volunteer/messages/${n._id}`)}
                      ...
                      >
                        ...
                        <div className="relative flex-shrink-0">
                          <div className="w-12 h-12 rounded-full bg-gradient-to-br from-teal-400 to-emerald-500 flex items-center justify-center text-white shadow-sm">
                            <Building2 size={20} />
                          </div>
                          {n.unread > 0 && (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2">
                            <p className={`text-sm font-bold truncate ${n.unread > 0 ? 'text-slate-900' : 'text-slate-700'}`}>
                              {n.name || 'Organization'}
                            </p>
                            {n.latestAt && (
                              <span className="text-[11px] text-slate-400 font-medium flex-shrink-0 flex items-center gap-1">
                                <Clock size={10} /> {timeAgo(n.latestAt)}
                              </span>
                            )}
                          </div>
                          ...
                          {n.latestMessage && (
                            <p className={`text-xs mt-1 truncate ${n.unread > 0 ? 'text-slate-700 font-semibold' : 'text-slate-400'}`}>
                              {n.latestMessage}
                            </p>
                          )}
                        </div>
